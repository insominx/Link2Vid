...
def get_error_guidance(err_text: str) -> tuple[str, str, str] | None:
    ...
    if reason == "js runtime":
        return (
            "js runtime",
            "JavaScript challenge solver failed",
            "yt-dlp needs a supported JavaScript runtime (deno/node/bun) and current\n"
            "EJS solver scripts for some downloads.\n\n"
            "Update yt-dlp and yt-dlp-ejs, confirm a JS runtime is on PATH, then retry.",
        )
    ...
    if reason == "network/rate-limit":
        return (
            ...
            "using browser cookies or cookies.txt. If this keeps happening,\n"
