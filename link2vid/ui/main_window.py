...
class VideoDownloaderApp:
    def __init__(self, root):
        ...
        self.fetcher = VideoFetcher(get_video_info=self.download_manager.get_video_info, log=self.log)
            ...
            if isinstance(outcome, NeedsSelenium):
                ...
                if self.ui_confirm("Fallback", "Automatic scrape failed. Try browser automation?"):
                    ...
                    if m3u8_url:
                        ...
                        self._prompt_hls_download(
                            playlist_url=m3u8_url,
                            headers={"Referer": url},
                            outfile_name="video.mp4",
                            confirm_message="Download with ffmpeg?",
                            missing_folder_title="Folder Error",
                            missing_folder_message="Please select a folder first.",
                        )
                    else:
    ...
    # Direct HLS helper
    # ──────────────────────────────────────────────────────────
    def _handle_direct_hls_result(self, result):
        return self._prompt_hls_download(
            playlist_url=result.playlist_url,
            headers=result.headers,
            outfile_name="gdcvault.mp4",
            confirm_message="Download with ffmpeg now?",
            missing_folder_title="Folder",
            missing_folder_message="Choose a download folder first.",
        )

    def _prompt_hls_download(
        self,
        *,
        playlist_url: str,
        headers: dict | None,
        outfile_name: str,
        confirm_message: str,
        missing_folder_title: str,
        missing_folder_message: str,
    ) -> bool:
        if not self.ui_confirm("Download", confirm_message):
            return False
