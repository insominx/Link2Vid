...
def run() -> None:
    ...
    def handle_transcript(card: VideoCard, transcript) -> None:
        card.set_status("Queued transcript")
        label = getattr(transcript, "label", None) or "auto-select"
        log_drawer.append(f"Queued transcript: {card.get_title()} [{label}]")
    ...
    samples = [
        {
            ...
            "meta": "Video site · 12:34 · Channel Name",
        },
        {
